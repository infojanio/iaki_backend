...
describe("Fetch User Check-in History Use Case", () => {
  ...
  it("Deve ser possível obter o histórico de pedidos", async () => {
    ...
    expect(orders).toEqual([
      expect.objectContaining({ storeId: "loja-01" }),
      expect.objectContaining({ storeId: "loja-02" }),
    ]);
  });
  ...
  it("Deve ser possível obter a lista paginada do histórico de pedidos.", async () => {
    ...
    expect(orders).toHaveLength(2);
    expect(orders).toEqual([
      expect.objectContaining({ storeId: "store-21" }),
      expect.objectContaining({ storeId: "store-22" }),
    ]);
  });
});
